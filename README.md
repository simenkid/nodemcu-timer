...
```lua
local count = 0
local repeater = timer.setTimeout(function ()
    count = count + 1
...
The callbacks for immediate execution enques in the order in which they were created. All immediate callbacks in the queue will be invoked every tick. If you queue an immediate callback from inside an executing callback, that immediate callback won't be invoked until the next tick comes. Remember that do not shcedule a long task for immediate execution as possible.
...
<a name="API_set"></a>
### set(tid)
Use a timer other than id=6 as the internal timer. Be aware of that the callback queues will be cleared when you set a new timer. 
